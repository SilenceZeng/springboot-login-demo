...
public class AuthController {
    ...
    public Object auth() {
        ...
        if (loginUser == null) {
            ...
            return LoginResult.success("获取用户信息成功", true, loginUser);
        }
    }

    @PostMapping("/auth/register")
    @ResponseBody
    public LoginResult register(@RequestBody Map<String, String> body) {
        ...
        if (username.length() < 1 || username.length() > 15) {
            return LoginResult.failure("用户名不符合要求");
        }
        if (password.length() < 6 || password.length() > 16) {
            return LoginResult.failure("密码不符合要求");
        }
        try {
    ...
    public LoginResult login(@RequestBody Map<String, Object> usernameAndPassword) {
        ...
        try {
            ...
            User loginUser = userService.getUserByUsername(username);
            return LoginResult.success("登录成功", true, loginUser);
