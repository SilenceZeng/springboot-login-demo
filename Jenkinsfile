...
def normalCIBuild(String version) {
    ...
    sh("docker build . -t 8.134.79.72:5000/xdml-springboot:${version}")

    sh("docker push 8.134.79.72:5000/xdml-springboot:${version}")
...
def deployVersion(String version) {
     // sh "ssh root@8.134.79.72 'docker rm -f xdml && docker run --name xdml -d -p 8080:8080 8.134.79.72:5000/xdml-springboot:${version}'"
}
...
def rollback() {
    def dockerRegistryHost = "http://8.134.79.72:5000";
